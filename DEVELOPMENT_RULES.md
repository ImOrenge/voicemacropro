...
#### 📅 날짜 형식
- **필수**: `YYYY-MM-DD` 형식 사용
- **예시**: `2025-01-17`, `2025-12-25`

#### 🏭 기능분류 태그
| 태그 | 설명 | 예시 |
|------|------|------|
| `feat` | 새로운 기능 추가 | `2025-01-17-feat: 음성인식 시스템 추가` |
| `fix` | 버그 수정 | `2025-01-17-fix: 매크로 실행 오류 수정` |
| `update` | 기존 기능 개선/수정 | `2025-01-17-update: UI 레이아웃 개선` |
| `refactor` | 코드 리팩토링 | `2025-01-17-refactor: 데이터베이스 모듈 분리` |
| `docs` | 문서 수정 | `2025-01-17-docs: API 문서 업데이트` |
| `style` | 코드 스타일 변경 | `2025-01-17-style: 코드 포맷팅 적용` |
| `test` | 테스트 추가/수정 | `2025-01-17-test: 단위테스트 추가` |
| `init` | 프로젝트 초기화 | `2025-01-17-init: 프로젝트 구조 생성` |
...
```bash
# 커밋 이름: 간략하게
git commit -m "2025-01-17-feat: 매크로 관리 UI 추가" -m "
📍 변경된 파일들:
...
```bash
# 버그 수정 예시
git commit -m "2025-01-17-fix: 데이터베이스 연결 실패 문제 해결" -m "
🐛 문제 상황:
...
# ❌ 너무 간략한 메시지
git commit -m "2025-01-17-fix: 버그수정"

# ❌ 한글 타입 미사용 (가독성 저하)
git commit -m "2025-01-17-feat: add macro management" -m "
- Added MacroManagementWindow.xaml
- Modified MacroModel.cs
"

# ❌ 변경사항 설명 부족
git commit -m "2025-01-17-update: 코드 수정" -m "파일들 업데이트함"
```
...
git commit -m "2025-01-17-feat: 매크로 추가 기능 구현"

# ✅ 좋은 예: 버그 수정 단위
git add database.py
git commit -m "2025-01-17-fix: 데이터베이스 연결 오류 수정"
```
...
# ❌ 나쁜 예: 파일별로 개별 커밋
git add VoiceMacroPro/Views/AddMacroWindow.xaml
git commit -m "2025-01-17-feat: XAML 파일 추가"

git add VoiceMacroPro/Views/AddMacroWindow.xaml.cs
git commit -m "2025-01-17-feat: C# 파일 추가"

# ❌ 나쁜 예: 동작하지 않는 상태에서 커밋
git add . 
git commit -m "2025-01-17-feat: 개발 중인 기능 저장"
```
...
# 기능별 브랜치 생성 시 (선택사항)
feature/2025-01-17-voice-recognition
feature/2025-01-17-macro-management
```
